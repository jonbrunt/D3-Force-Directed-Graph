...
function main(dataSet) {
  ...
  simulation
    ...
    .force(
      "center",
      d3
        .forceCenter() // centers force on nodes to midpoint of svg
        .x(width / 2)
        .y(height / 2)
    )
    .force(
      "links",
      d3
        .forceLink(links) // sets link distances
        .distance(125)
    )
  ...
  $("#buttonOne").click(() => {
    simulation
      .force(
        "yForce",
        d3
          .forceY() // y force to horizontally separate nodes
          .strength(0.03)
          .y(height / 2)
      )
      ...
      .force(
        "charge",
        d3
          .forceManyBody() // adds repulsive charge
          .strength(-20)
      )
  ...
  $("#buttonTwo").click(() => {
    simulation
      ...
      .force(
        "charge",
        d3
          .forceManyBody() // adds attractive charge
          .strength(0.5)
      )
      .velocityDecay(0.3)
      .alpha(1) // reinitializes simulation
